...
describe('C3: Tag/metadata-based filtering', () => {
  ...
  it('no reqTags = today\'s behavior (all models eligible)', () => {
    const route = routeRequest(100, undefined, undefined, undefined, undefined, {});
  ...
  it('empty reqTags set = today\'s behavior', () => {
    const route = routeRequest(100, undefined, undefined, undefined, undefined, { reqTags: new Set() });
    expect(route).toBeDefined();
    route.release();
  });
  ...
  it('model with matching tag is selected when reqTags includes it', () => {
    ...
    const route = routeRequest(100, undefined, undefined, undefined, undefined, { reqTags: new Set(['fast']) });
    expect(route).toBeDefined();
  ...
  it('model with "default" tag is always eligible', () => {
    ...
    const route = routeRequest(100, undefined, undefined, undefined, undefined, { reqTags: new Set(['premium']) });
    expect(route).toBeDefined();
    ...
    const premiumRoute = routeRequest(100, undefined, undefined, undefined, new Set([models[0].id]), { reqTags: new Set(['premium']) });
  ...
  it('model with non-intersecting tags is skipped', () => {
    ...
    expect(() => {
      const r = routeRequest(100, undefined, undefined, undefined, undefined, { reqTags: new Set(['fast']) });
      r.release();
    }).toThrow();
  });
