...
// ── Per-platform settings for built-in providers ─────────────────────────────
// Built-ins (Groq, Cerebras, etc.) couldn't be edited through the dashboard
// until now. Their settings live in `built_in_provider_settings` (one row
// per built-in slug, seeded on boot). The router falls back to this row
// when a per-model limit in `models` is NULL.
...
keysRouter.get('/platforms/:platform/settings', (req: Request, res: Response) => {
  const platform = req.params.platform as string;
  if (!(PLATFORMS as readonly string[]).includes(platform)) {
...
keysRouter.patch('/platforms/:platform/settings', (req: Request, res: Response) => {
  const platform = req.params.platform as string;
  if (!(PLATFORMS as readonly string[]).includes(platform)) {
    res.status(404).json({ error: { message: `Unknown built-in platform '${platform}'` } });
    return;
