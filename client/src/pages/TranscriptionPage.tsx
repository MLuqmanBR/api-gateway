...
interface UsageData {
  families: { family: string; requestsToday: number; audioMinutesMonth: number }[]
}

function formatPrice(usd: number | null): string {
  if (usd === null) return '—'
  return `$${usd.toFixed(2).replace(/\.?0+$/, '')}/hr`
}
...
export default function TranscriptionPage() {
  ...
  // Local unsaved edits, same pattern as the embeddings page.
  ...
  const { data, isLoading } = useQuery<TranscriptionsData>({
    queryKey: ['transcriptions'],
    queryFn: () => apiFetch('/api/transcriptions'),
  })

  const { data: usage } = useQuery<UsageData>({
    queryKey: ['transcriptions', 'usage'],
    queryFn: () => apiFetch('/api/transcriptions/usage'),
  ...
  const saveMutation = useMutation({
    mutationFn: (body: { defaultFamily?: string; providers?: { id: number; priority: number; enabled: boolean }[] }) =>
      apiFetch('/api/transcriptions', { method: 'PUT', body: JSON.stringify(body) }),
    onSuccess: () => {
  ...
  return (
    <div>
      <PageHeader
        title="Models"
        description="Audio transcription fails over within a family only: the same model served by another provider. Transcripts from different models are incompatible, so the router never swaps models on you."
        divider={false}
        actions={<ModelsTabs />}
      />
      ...
      <div className="space-y-6">
        <p className="text-xs text-muted-foreground">
          ...
          <code className="rounded-md bg-muted px-1.5 py-0.5 font-mono">POST /v1/audio/transcriptions</code> routes to the
          ...
          <>
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <ModelSearchBox
                ...
                placeholder="Filter transcription models by family, provider or model id…"
              />
                ...
                <section key={f.family} className={`rounded-3xl border bg-card p-5 ${noKeys ? 'opacity-60' : ''}`}>
                  <div className="flex items-baseline justify-between gap-4 mb-3 flex-wrap">
                    ...
                    <span className="text-xs text-muted-foreground tabular-nums">
                      {u ? <>{u.requestsToday} req today · {formatMinutes(u.audioMinutesMonth)} audio this month</> : '—'}
                    </span>
                  </div>
