...
export default function AnalyticsPage() {
  ...
  const { data: errors = [] } = useQuery({
    ...
    queryFn: () => apiFetch<ErrorLogEntry[]>(`/api/analytics/errors?range=${range}`),
  })
  ...
  const { data: summary30 } = useQuery({
    ...
    queryFn: () => apiFetch<AnalyticsSummary>(`/api/analytics/summary?range=30d`),
  })
  const actualSavings = summary?.estimatedCostSavings ?? 0
  const baseSavings = summary30?.estimatedCostSavings ?? 0
  // Capture "now" once at mount via the lazy useState initializer: reading
  // Date.now() directly during render is impure (flagged by react-hooks), and
  // the projection is coarse enough (whole days) that a mount-time snapshot is
  // indistinguishable from a per-render read.
